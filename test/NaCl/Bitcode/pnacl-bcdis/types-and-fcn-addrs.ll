...
; CHECK-NEXT:      58:6|    2: <65533, 11, 2, 1, 1,  |      @a0 = abbrev <1, fixed(5)>;
; CHECK-NEXT:          |        0, 1, 5>             |
...
; CHECK-NEXT:          |        6, 0, 1, 5, 0, 1, 4> |                   fixed(5), fixed(4)>;
...
; CHECK-NEXT:          |        1, 1, 0, 3, 0, 1, 5> |                  array(fixed(5))>;
...
; CHECK-NEXT:     165:1|    4: <21, 0, 4>            |    @t7 = void (); <%a0>
...
; CHECK-NEXT:     178:0|    4: <21, 0, 10, 0, 0>     |    @t11 = i64 (i32, i32); <%a0>
; CHECK-NEXT:     181:1|    4: <21, 0, 3, 1>         |    @t12 = double (float); <%a0>
; CHECK-NEXT:     183:5|    4: <21, 0, 1, 3>         |    @t13 = float (double); <%a0>
; CHECK-NEXT:     186:1|    4: <21, 0, 4, 0>         |    @t14 = void (i32); <%a0>
; CHECK-NEXT:     188:5|    4: <21, 0, 2, 2, 2>      |    @t15 = 
; CHECK-NEXT:          |                             |        <4 x i32> 
; CHECK-NEXT:          |                             |        (<4 x i32>, <4 x i32>); <%a0>
; CHECK-NEXT:     191:6|    4: <21, 0, 0, 0>         |    @t16 = i32 (i32); <%a0>
...
; CHECK-NEXT:     252:0|    6: <1, 1, 116, 101, 115, |    @f1 : "test"; <@a2>
; CHECK-NEXT:          |        116>                 |
; CHECK-NEXT:     257:1|    6: <1, 2, 109, 101, 114, |    @f2 : "merge"; <@a2>
; CHECK-NEXT:          |        103, 101>            |
; CHECK-NEXT:     263:0|    6: <1, 3, 70, 108, 111,  |    @f3 : "Float2Double"; <@a2>
; CHECK-NEXT:          |        97, 116, 50, 68, 111,|
; CHECK-NEXT:          |        117, 98, 108, 101>   |
; CHECK-NEXT:     274:1|    6: <1, 4, 68, 111, 117,  |    @f4 : "Double2Float"; <@a2>
; CHECK-NEXT:          |        98, 108, 101, 50, 70,|
; CHECK-NEXT:          |        108, 111, 97, 116>   |
; CHECK-NEXT:     285:2|    6: <1, 5, 105, 110, 100, |    @f5 : "indefargs"; <@a2>
; CHECK-NEXT:          |        101, 102, 97, 114,   |
; CHECK-NEXT:          |        103, 115>            |
; CHECK-NEXT:     294:1|    6: <1, 0, 102, 117, 110, |    @f0 : "func"; <@a2>
; CHECK-NEXT:          |        99>                  |
; CHECK-NEXT:     299:2|    5: <1, 7, 102, 111, 111, |    @f7 : {'f', 'o', 'o',  36, 'b', 
; CHECK-NEXT:          |        36, 98, 97, 114>     |           'a', 'r'} <@a1>
; CHECK-NEXT:     307:4|    6: <1, 6, 77, 117, 108>  |    @f6 : "Mul"; <@a2>
