...
define i16 @fold_global(i32 %i) {
  %add = add i32 %i, 34
  %arrayidx = getelementptr %struct.A* bitcast ([72 x i8]* @global to %struct.A*), i32 0, i32 1, i32 %add
  %t0 = load volatile i16* %arrayidx, align 2
  ret i16 %t0
}
...
define i16 @no_reassociate(%struct.A* %p, i32 %i) {
  %add = add i32 %i, 34
  %arrayidx = getelementptr %struct.A* %p, i32 0, i32 1, i32 %add
  %t0 = load volatile i16* %arrayidx, align 2
  ret i16 %t0
}
