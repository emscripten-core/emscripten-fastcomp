...
// describing a static alloc memory region suitable for our analysis.
// If so, return the alloca, otherwise return NULL.
const AllocaInst *
...
void AllocaManager::collectMarkedAllocas() {
  ...
  // Weird semantics: If an alloca *ever* apperas in a lifetime start or end
  ...
  // block, so that we can replace later ones with earlier ones and skip worrying
  // about dominance, so run through the entry block and index those allocas
...
// Decide which allocas will represent which other allocas, and if so what
// their size and alignment will need to be.
void AllocaManager::computeRepresentatives() {
  ...
  for (size_t i = 0, e = AllocasByIndex.size(); i != e; ++i) {
    // If we've already replaced this alloca, don't visit it.
...
void AllocaManager::computeFrameOffsets() {
  ...
  // ones with no lifetime markers that we haven't looked at yet. We walk
  // in reverse order so that we can set the representative allocas as those
  // that dominate the others as we go.
  const BasicBlock *EntryBB = &F->getEntryBlock();
  for (BasicBlock::const_iterator BI = EntryBB->begin(), BE = EntryBB->end();
       BI != BE; ++BI) {
    const AllocaInst *AI = dyn_cast<AllocaInst>(BI);
    ...
    if (I != Allocas.end()) {
      // An alloca with lifetime markers. Emit the record we've crafted for
      // it, if we've chosen to keep it as a representative.
  ...
  for (SmallVectorImpl<AllocaInfo>::const_iterator I = SortedAllocas.begin(),
    ...
    // For backwards compatibility, align every power-of-two multiple alloca
    // to its greatest power-of-two factor, up to 8 bytes. In particular,
    // cube2hash is known to depend on this.
  ...
  // Add allocas that were represented by other allocas to the StaticAllocas
  // map so that our clients can look them up.
...
void AllocaManager::analyze(const Function &Func, const DataLayout &Layout) {
  ...
  // Get the declarations for the lifetime intrinsics so we can quickly test
  // to see if they are used at all, and for use later if they are.
